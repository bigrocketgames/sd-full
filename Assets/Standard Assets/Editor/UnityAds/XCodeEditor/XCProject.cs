...
namespace UnityEngine.Advertisements.XCodeEditor
{
	public partial class XCProject : System.IDisposable
	{
		...
		public PBXDictionary AddFile(string filePath, PBXGroup parent, string tree, bool createBuildFiles, bool weak)
		{
		...
		public bool AddFolder(string folderPath, PBXGroup parent, string[] exclude, bool recursive, bool createBuildFile)
		{
			...
			foreach(string directory in Directory.GetDirectories( folderPath )) {
				...
				if(directory.EndsWith(".bundle")) {
					...
					AddFile(directory, newGroup, "SOURCE_ROOT", createBuildFile, false);
					Debug.Log("fatto");
					continue;
				}
			...
			foreach(string file in Directory.GetFiles( folderPath )) {
				...
				AddFile(file, newGroup, "SOURCE_ROOT", createBuildFile, false);
			}
		...
		public PBXGroup GetGroup(string name, string path, PBXGroup parent)
		{
			if(string.IsNullOrEmpty(name))
				return null;
			...
			PBXGroup result = new PBXGroup(name, path, "SOURCE_ROOT");
			groups.Add(result);
		...
		{
			PBXGroup modGroup = this.GetGroup(mod.group, null, null);
			...
			PBXGroup frameworkGroup = this.GetGroup("Frameworks", null, null);
			...
			if(mod.files != null) {
				foreach(string filePath in mod.files) {
					...
					this.AddFile(MacPath(absoluteFilePath), modGroup, "SOURCE_ROOT", true, false);
				}
			}
			...
			if(mod.folders != null) {
				foreach(string folderPath in mod.folders) {
					...
					this.AddFolder(MacPath(absoluteFolderPath), modGroup, (string[])mod.excludes.ToArray(), true, true);
				}
			}
